...
from src_torch.models import Mnist_2NN, Mnist_CNN
from src_torch.clients import ClientsGroup, client
...
if __name__=="__main__":
    ...
    if is_test:
        with torch.no_grad():
            state_dict = torch.load("checkpoints/mnist_2nn_num_comm19_E2_B64_lr0.01_num_clients10_cf1.pkl")
    ...
    for i in range(args['num_comm']):
        ...
        if (i + 1) % args['save_freq'] == 0:
            torch.save(net.state_dict(), os.path.join(args['save_path'],
                                         '{}_num_comm{}_E{}_B{}_lr{}_num_clients{}_cf{}.pkl'.format(args['model_name'],
                                                                                                i, args['epoch'],
                                                                                                args['batchsize'],
                                                                                                args['learning_rate'],
                                                                                                args['num_of_clients'],
